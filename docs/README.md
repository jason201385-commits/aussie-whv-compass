...
> 版本 2.0｜最後更新 2026-09-17｜接手 agent 從這一頁開始，不要從「讀全文」開始。
...
| 檔案 | 管什麼 | 什麼時候改 |
|---|---|---|
| [`TASK_FIRST_ROLLOUT.md`](TASK_FIRST_ROLLOUT.md) | 新手任務答案的來源維護、真人測試與待發推廣素材 | 任務答案或測試流程變動 |
| [`AI_DISCOVERY.md`](AI_DISCOVERY.md) | AI／Claude 探索格式、Cloudflare 只讀排查與驗收邊界 | 爬取或機器閱讀格式變動 |
...
## 4a. 走網路的檢查（不在 `check.ps1` 裡）

`check.ps1` 必須離線、可重現，所以會連外的檢查單獨跑：

```
node scripts/check_links.mjs           # 全站外部連結，約 3 分鐘
node scripts/check_links.mjs --only ato
```

輸出分兩類：`BLOCKED` 是對方的 bot 阻擋或限流（403／429），不算問題；
`CHECK` 是待人工複驗的候選。**狀態碼證明不了一個頁面死了**——ABN Lookup、AUSTRAC 與
Google 說明中心都會對非瀏覽器回 404，瀏覽器卻正常，所以每一條 `CHECK` 都要用真實瀏覽器
開過才下判斷：真的沒了就改連結（並依 Hard Constraint #15 處理隨之無法查證的內容），
只是 WAF 擋機器就把主機加進腳本的 `CONFIRMED_ALIVE` 並註明確認日期。
背景與第一次掃描的結果見 `DECISIONS.md` D-2026-09-04-04。
