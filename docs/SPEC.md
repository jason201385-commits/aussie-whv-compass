...
> 版本 2.0｜最後更新 2026-09-17｜本文件只寫「現在的行為是什麼、怎麼驗證」。
...
### 1.1 頁面清單（根層內容頁＋404；以本表及產生器清單為準）
...
| 安全列（P0-8） | 首頁 `<nav id="support-hub" class="safety-bar">`（header 之後、hero 之前，常駐不收合） | 5 個零打字直達連結，前綴「很急？」 | 只連既有內容錨點，依序：受傷 → `health.html#emergency`；剛匯款 → `scam.html#help`；被威脅或扣證件 → `scam.html#help`；簽證到期 → `visa.html#apply`；今晚沒地方住 → `housing.html#housing-search-tool`；任何狀態一鍵可達、鍵盤可操作；`assist.ts` 目錄與各面板「急事先走安全出口」都連到 `#support-hub`；完整說明留在各目的頁 | 先做降低傷害的第一步，不複製可能過時的電話或政策數字 | check.ps1「P0-8 安全列」區塊（恰好 5 個 `<a>`、順序、無 `<details>`）；`scripts/clarifier-contract.mjs` 靜態契約；常駐高度量測值見 DECISIONS D-2026-09-03-02 |
| 首頁釐清器（P0-7／P0-8／P0-10） | 首頁 `#clarifier`：`nav#journey-map`（4 階段 chips，緊接 hero）→ 信任列一句 → 4 個 `.clarifier-panel`（`#considering`、`#committed`、`#in-australia`、`#next-step`；h2 以階段 chip 文案開頭）→ 護照 radiogroup（前兩個面板）→ 需求 chips → `.clarifier-exit` | 階段 chips 4（台灣預設：還在考慮／決定要去／已在澳洲／回程或留下；462 版：還在糾結／決定要去（等抽籤也算）／已經到澳／回程或留下）；護照 `role="radiogroup"` 三顆 `role="radio"`（台灣 417／中國大陸 462／其他；只在前兩個面板，JS 才顯示，無 JS 顯示一句靜態連結；方向鍵換選項、空白鍵選取、`aria-checked` 同步）；需求 chips 6／8／10／6（8 字口語版，含「看公開討論」「全部看」） | hash 驅動、零儲存：`#<stage>` 只顯示該面板並把焦點移到標題；`#exit-*` 只顯示該出口；`#<stage>-exits` 顯示該面板全部出口；`aria-current="step"`／`aria-checked` 只在執行期；選 462 後以 `data-passport` 換字（階段 chips 4＋需求 chips 30＋出口標題 21 = 55 個 `data-label-462`，全部繁體字）、改寫 `data-href-462` 連結並顯示 462 摘要卡（HTML 初始不帶 `hidden`，JS 只在非 462 時加 `hidden`），切回 417 還原；不寫 storage、不 fetch；無 JS 時 4 面板、21 個出口與摘要卡全部可見、chips 皆為錨點；每個面板附「急事先走安全出口」 | 21 個出口（3／6／8／4）：12 張回收的問題卡（保留 `problem-category`／`card-action`）＋9 個一句話精簡出口，各附深連結與「看公開討論」（P1-21 前連 `#communities`）及固定安全句「不配對、不代聊。這裡只放公開入口；找房找工請走平台搜尋，不要先傳護照或匯款。」；出口 3、11、12 與 462 摘要卡附 passport.md §10 短版邊界句；462 摘要卡（considering／committed 各一張）：標題、四行、一句、按鈕 `lang/en/visa/#choose`、次要官方連結、來源列 | check.ps1「首頁首屏」「首頁釐清器」「P0-10 護照 radiogroup」「P0-10 462 摘要卡」「P0-10 換字契約」「P0-8 收尾」區塊；`scripts/clarifier-contract.mjs`（靜態契約＋node:vm 行為契約，check.ps1 執行）；瀏覽器量測值見 DECISIONS D-2026-09-03-02 |
...
| 分層證據卡 | 9 頁（visa、cost、work、health、scam、housing、leave、market、pr）；其中 P0-11 五頁的四列 meta 收進答案卡 `<details class="answer-card-evidence">` | — | 白話下一步 → 理由 → 來源機構、查核日期、編輯狀態常駐；`data-evidence-status="checked|stale"`；`<details>` 展開，不依賴 JS | 過期標「待重新確認」並保留官方出口 | check.ps1「高風險主題必須先給安全下一步」與「P0-11」區塊 |
...
5. 固定案例回放：`scripts/test_housing_search.mjs`（17 案例）、`scripts/test_tools.mjs`（集簽快查器 7 案例＋試算器基準，21 案例）、
   `scripts/test_analytics.cjs`（GA4 敏感頁排除，187 斷言）、`scripts/test_job_router.mjs`（公開求職篩選導流，不含雇主名單）。
